...
#include <isaacsim/asset/importer/urdf/IUrdf.h>
#include <isaacsim/asset/importer/urdf/ImportHelpers.h>
#include <isaacsim/asset/importer/urdf/UrdfImporter.h>
                        ...
                        const bool getArticulationRoot = false)
{
    ...
    if (!stages["stage"]) // If all else fails, import on current stage
    {
        ...
        if (pxr::TfStringStartsWith(identifier, "anon:"))
        {
            ...
            stages["robot_stage"] = stages["stage"];
            multi_layer = false;
            save_stage = false;
        }
        else
    ...
    if (stages["stage"])
    {
                                           ...
                                           // lost and simualtion won't work
        {
            ...
            stages["stage"]->Save();
            CARB_LOG_INFO("Saving Robot Stage %s", stages["robot_stage"]->GetRootLayer()->GetIdentifier().c_str());
